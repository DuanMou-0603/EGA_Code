...
void init()
{
    ...
    //将 COMM1_GPIO_Port 的 COMM1_Pin 设置为高电平（ GPIO_PIN_SET ），这是ASK调制通信的输出引脚


    askData.askLoopIndex = 0;
    //状态机复位 ：将 askData.askLoopIndex 设置为0，重置ASK通信的状态机索引
}
...
{
    if(!askData.enableASK)//- 当ASK通信被禁用时，将通信引脚拉低并重置状态机
    {
    ...
    case 0:
        ...
        packData(adcData.pWPTlf, askData.powerRequirement);//调用 packData() 打包数据
    ...
    case 39:
        ...
        break;
    default:
        if(askData.askLoopIndex & 0b1)//- 每2个索引传输1个数据位
        //- 根据 txMessage 中对应位的值决定是否翻转GPIO状态
        {
