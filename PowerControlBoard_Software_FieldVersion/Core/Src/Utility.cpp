...
void IncreasementPID::setClamp(float _lower, float _upper)//输出限幅
{
    clamp_lower = _lower;
    clamp_upper = _upper;
    clamp_enabled = true;
...
//传统增量式PID Δu(k) = Kp[e(k) - e(k-1)] + Ki*e(k) + Kd[e(k) - 2*e(k-1) + e(k-2)]
...
void IncreasementPID::computeDelta(float _target, float _current)
{
    ...
    if(clamp_enabled)
        this->deltaOutput = M_CLAMP(this->deltaOutput, clamp_lower, clamp_upper);//输出限幅
