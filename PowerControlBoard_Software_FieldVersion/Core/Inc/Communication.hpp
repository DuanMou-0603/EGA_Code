...
#define ASK_COMM_FREQ           2000U//ASK基频为2000Hz
#define ASK_COMM_TIM            &htim8
#define ASK_COMM_TIM_CHANNEL1   TIM_CHANNEL_1
//使用TIM8的通道1和通道2进行信号输出

#define ASK_COMM_BIT_HIGH       0x8000//表示数字"1"的高电平幅度值
#define ASK_COMM_BIT_LOW        0x0000//表示数字"0"的低电平幅度值

#define ASK_COMM_PERIOD         20U//通信周期为20个单位


struct ASKData
{
    bool enableASK = true; // 是否启用ASK通信
    uint16_t txMessage;//待发送的12位数据包，包含起始位、数据位、校验位和停止位
    uint8_t askLoopIndex;//ASK通信状态机索引，用于控制数据包的逐位发送时序
    ...
    uint16_t lowPowerCnt = 0;//当WPT功率低于3W时递增，超过150次时会关闭WPT输出
    uint32_t lastPowerOnTime = 0;//上次开启功率传输的时间戳，用于控制重启间隔

    bool allowRestart = 0;//重启允许标志，当WPT电压低于电池电压且电容电压充足时允许重新启动WPT
};
